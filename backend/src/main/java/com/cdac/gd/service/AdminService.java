...
public class AdminService {
    ...
    public Map<String, Object> getServiceHealth() {
        ...
        if (githubToken != null && !githubToken.isEmpty() && !githubToken.contains("placeholder")) {
            ...
            aiHealth.put("responseTime", 10); // Mock time for config check
        } else {
            aiHealth.put("status", "warning"); // Config missing
            aiHealth.put("responseTime", 0);
        }
        aiHealth.put("lastCheck", LocalDateTime.now().toString());
        services.put("ai", aiHealth);
